...
namespace gloox
{
  ...
  class GLOOX_API MUCInvitationHandler
  {
    public:
      ...
      virtual void handleMUCRegister(const JID & room, const JID & from) = 0;

      virtual void handleMUCAgree(const JID & room, const JID & from, const JID & to) = 0;
      
      virtual void handleMUCDisagree(const JID & room, const JID & from, const JID & to) = 0;
  };
